...
def generate_encodings():
    ...
    for image_path in Path(PHOTOS_DIR).rglob('*'):
        ...
        try:
            ...
            if len(face_locations) > 1:
                print(f"Multiple faces found in {image_path.name}. Using the first face...")
            
            # Generate face encodings
            face_encodings = face_recognition.face_encodings(image, face_locations)
            
            if face_encodings:
                # Add the first face encoding and name to our lists
                # Only add if this person's encoding hasn't been added yet
                if student_name not in known_names:
                    known_encodings.append(face_encodings[0])
                    known_names.append(student_name)
                    print(f"Successfully encoded {student_name}'s face")
                else:
                    print(f"Skipping additional photo for {student_name} - already encoded")
            else:
    ...
    try:
        ...
        with open(ENCODINGS_FILE, "wb") as f:
            pickle.dump(data, f)
        print(f"\nSuccessfully generated encodings for {len(known_names)} students:")
